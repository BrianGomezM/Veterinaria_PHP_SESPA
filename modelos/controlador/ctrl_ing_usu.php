...
header("Content-Type: text/html; charset=iso-8859-1");
	...
	case 'reg':
		...
		FROM usuario 
		WHERE usuario.cedula_usu = '$var_ident'";
		$res=mysql_query($sql);
		echo "SQL: $sql<BR>";
		while($row=mysql_fetch_array($res))
			{
				$var_1=$row[cedula_usu];
				echo "$var_1";
			}
		if ($var_1>0)
			{
		...
		else
			{
					...
					echo "Ape:  $var_p_apellido<br>";
					$var_s_apellido=$_POST[apel2];
					echo "Ape:  $var_s_apellido<br>";
					$var_direccion=$_POST[dir];
					echo "Ape:  $var_direccion<br>";
					$var_telefono=$_POST[tel];
					echo "Ape:  $var_telefono<br>";
					echo "edad:  $var_edad<br>";
				    $var_fecha_naci= date("y:m:d");
					echo "fecha:  $var_fecha_naci<br>";
					$var_email=$_POST[corre];
					echo "Email: $var_email<br>";
					$var_contrasena=$_POST[pass];
					echo "Pass:  $var_contrasena<br>";
					$var_cc=$_POST[iden];
					echo " Ident:  $var_cc<br>";
					$var_id_roll=$_POST[area];
					echo "Roll: $var_id_roll<br>";
					$var_tipo_doc=$_POST[tipodoc];
					echo "Tipo_doc: $var_tipo_doc<br>";
					$var_profesion=$_POST[profesion];
					echo "Tipo_doc: $var_profesion<br>";
					
					$sql="insert into usuario(nom_usu, apellido1_usu,apellido2_usu,direccion_usu,edad_usu, cedula_usu,telefono_usu, correo_usu, pass_usu,fecha_registro_usu, id_roll, id_profesion_propie,tipo_id_usu)
					                 values  ('$var_nom', '$var_p_apellido', '$var_s_apellido','$var_direccion','$var_edad', '$var_cc','$var_telefono','$var_email', '$var_contrasena', '$var_fecha_naci', '$var_id_roll', '$var_profesion','$var_tipo_doc')";
					ECHO "$sql<BR>";
					$est=mysql_query($sql);
					if(!$est)
						{
						...
						else
						{
							...
							ECHO " <script>parent.document.register_form.pass.value=''</script> ";
							ECHO " <script>parent.document.register_form.pass2.value=''</script> ";
							ECHO " <script>parent.document.register_form.edad.value=''</script> ";
