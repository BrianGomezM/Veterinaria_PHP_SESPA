...
<script type="text/javascript">
						...
						function Registrar(register_form,aux)
							{
								...
								if(document.register_form.enfer.value=='')
									{
	...
	<body bgcolor="#ffffff"> <div id="ventana1">
	</div>			
			<table border="0" align="center" width='90%' cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width='1%' class="td_titulo" align="right">
						<!--<button title="Salir" ><IMG  SRC="../../IMAGENES/salir1.png"></IMG></button>-->
						<input	type='button' name='btn_cerrar' class='boton1' title="Cerrar Sesion" 
				...
				<tr>
					...
					<div id="register-div">
					<form  name="register_form" method="post" target="operaciones" action="../controlador/ctrl_ing_enfer.php">
					<input type="hidden" id="action" name="action" >
					
							<fieldset id="fiel"><legend id="regis" align="center">ENFERMEDAD</legend> 
								<input type="hidden" id="id" name="id" >
							<label>
								Nombre:<br>
							<input type="text" name="enfer" id="enfer" title="Digite el nombre de la enfermedad"/>  
						<span></span>
					</label>
						<label>
								Sintoma:<br>
								...
								<select type="text" NAME="area">
									<option value="0">Seleccione..</option>
										<?php
											$query=" select id_sin, descripcion_sin,localizacion_sin from sintoma";
												$res=mysql_query($query);
												while($row=mysql_fetch_array($res))
													{
													echo "<option value=".$row[id_sin]." > ".$row[descripcion_sin]."</option>";
													}
										?>
								</select>	
								</label>
			<!--<input type="button" onclick="validar(this.form)" value="Grabar usuario" class="boton">-->
				...
				<input type="button" id="btn_query" name="btn_query" value="Consultar"  Onclick="ventana1('consulta_enfermedad.php', 1000, 400, 'CONSULTAR DATOS DE ENFERMEDAD');return false;" >
  </fieldset>  
