...
header("Content-Type: text/html; charset=iso-8859-1");
	...
	require_once('../etiquetas/etq_usuario.php'); // ruta relativa a include/config.php
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script src="../../herramientas/jquery/js/prototype.js" type="text/javascript"></script>
					...
					return false;
						...
						function Registrar(register_form,aux)
						
							{
								...
								if(document.register_form.dir.value=='')
									{
										alert("Falta digitar la DIRECCION");
									}
								if(document.register_form.tel.value=='')
									{
										alert("Falta digitar el TELEFONO");
									}
								if(document.register_form.corre.value=='')
									{
										alert("Falta digitar el CORREO ELECTRONICO");
									}
								if(document.register_form.tipodoc.value=='0')
									{
										alert("Falta digitar el TIPO DE DOCUMENTO");
									}
								if(document.register_form.iden.value=='')
									{
										alert("Falta digitar el NUMERO DE IDENTIFICACION");
									}
								if(document.register_form.pass.value=='')
									{
										alert("Falta digitar el PASSWORD");
									}
								if(document.register_form.pass2.value=='')
									{
										alert("Por favor comprobar el PASSWORD");
									}
								else
									{
	...
	<div id="ventana1"></div>
		...
		<div id="register-div">
					...
					<input type="hidden" id="action" name="action" >
							<fieldset id="fiel"><legend id="regis" align="center"><?php echo"$etq_regi_usu" ?> </legend>
					...
					<label>
						...
						<input type="text" style="cursor:pointer" name="calendar" id="calendar" size="9%" title="<?php echo "$title_fecha" ?>" readonly />
						<span></span>
					</label>
						<label>
				...
				<label>
							...
							<br>
								<select type="text" style="cursor:pointer;" NAME="tipodoc" title="<?php echo"$title_tipo_doc" ?>">
					...
					<label>
					<?php echo"$etq_contra" ?><br>
								<input type="password" name="pass" id="pass" title="<?php echo"$title_contra" ?>"/>
						<span></span>
					</label>
					<label>
						<?php echo"$etq_conf_contra" ?><br >
						<input type="password" name="pass2" id="pass2"  title="<?php echo"$title_c_contra" ?>"/>

							<span></span>
						</label>
							<label>
						<?php echo"$etq_profesi" ?>
							
							<br>
								<select style="cursor:pointer;" type="text" NAME="profesion" title="<?php echo"$title_profesi" ?>">
									<option value="0"><?php echo"$etq_seleccione" ?>
</option>
										<?php
											$query=" select id_profesion_propie, nom_profe_propie from profesion_propie ";
								...
								<button	type="button" id="n_pro" width="2%" name="n_pro" value="nueva pro"  Onclick="ventana1('agregar_profe.php', 1200, 600, 'AGREGAR PROFESION');return false;" title="AGREGAR NUEVA PROFESION" style="width:80px; cursor:pointer">	
						...
						<?php echo"$etq_roll" ?>
							...
							<br>
								<select style="cursor:pointer;" type="text" NAME="area" title="<?php echo"$title_roll" ?>">
									<option value="0"><?php echo"$etq_seleccione" ?>
</option>
										<?php
											$query=" select id_roll, nombre_roll from roll ";
												$res=mysql_query($query);
												while($row=mysql_fetch_array($res))
													{
													echo "<option value=".$row[id_roll]." > ".$row[nombre_roll]."</option>";
													}
										?>
								</select>
								</select>
								<button	type="button" id="n_ro" width="2%" name="n_ro" value="nuevo roll"  Onclick="ventana1('agregar_roll.php', 1200, 600, 'AGREGAR ROLL');return false;" title="AGREGAR NUEVO ROLL" style="width:80px; cursor:pointer" >	
									<img src="../../imagenes/role.png">
								</button>
						</label>
						<br>
			<!--<input type="button" onclick="validar(this.form)" value="Grabar usuario" class="boton">-->

				<input type="button" id="btn_regis" name="btn_regis" value="<?php echo"$boton_guardar" ?>"  Onclick="Registrar(this.form,'reg')" style="width:80px; cursor:pointer" >
				<input type="button" id="btn_update" name="btn_update" value="<?php echo"$boton_actualizar" ?>"  Onclick="Registrar(this.form,'updt')" style="width:80px; cursor:pointer">
				<input type="button" id="btn_dell" name="btn_dell" value="<?php echo"$boton_eliminar" ?>"  Onclick="eliminar(this.form,'dell')" style="width:80px; cursor:pointer">
				<input type="button" id="btn_query" name="btn_query" value="<?php echo"$boton_consultar" ?>"  Onclick="ventana1('consulta_usu.php', 1200, 600, 'CONSULTAR DATOS DE USUARIO');return false;" style="width:80px; cursor:pointer">
